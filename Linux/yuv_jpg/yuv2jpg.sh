    ...
    if [[ $line =~ ([^ ]+)\ ([^ ]+)\ (stage1[^ ]+)(\ (stage2[^ ]+))* ]]; then
        ...
        then
            if [[ $stage1 =~ 'stage1:T' && $stage2 =~ 'stage2:F' ]]; then
                echo "stage1"
                \ffmpeg -loglevel quiet -y -s 640x360 -i $yuv_file ./${stage1_jpg}/${yuv_file%.*}_$version.jpg < /dev/null
        		cp $yuv_file ./stage1/stage1_yuv
		        echo $line >> stage1/call_stage1_$version.rect
            elif [[ $stage1 =~ 'stage1:T' && $stage2 =~ 'stage2:T' ]]; then
                echo "stage1 & stage2"
                \ffmpeg -loglevel quiet -y -s 640x360 -i $yuv_file ./${stage2_jpg}/${yuv_file%.*}_$version.jpg < /dev/null
